...
func RefreshTidalAPIList(force bool) ([]string, error) {
	...
	if !containsTidalAPIURL(state.URLs, state.LastUsedURL) {
		state.LastUsedURL = ""
	}
...
func GetRotatedTidalAPIList() ([]string, error) {
	...
	return rotateTidalAPIURLs(state.URLs, state.LastUsedURL), nil
}
...
func containsTidalAPIURL(values []string, target string) bool {
