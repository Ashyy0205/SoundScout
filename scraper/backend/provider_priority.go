...
// prioritizeProviders sorts providers by past success so the previously-working
// endpoint is tried first, avoiding timeout waits on known-dead hosts.
...
func providerPriorityKey(service, provider string) string {
...
func recordProviderOutcome(service string, provider string, success bool) {
	...
	serviceKey := strings.TrimSpace(strings.ToLower(service))
	pKey := providerPriorityKey(serviceKey, provider)
	now := time.Now().Unix()
	...
	if err := providerPriorityDB.Update(func(tx *bolt.Tx) error {
		...
		if raw := bucket.Get([]byte(pKey)); len(raw) > 0 {
			if err := json.Unmarshal(raw, &entry); err != nil {
				return err
			}
		}
		...
		return bucket.Put([]byte(pKey), payload)
	}); err != nil {
