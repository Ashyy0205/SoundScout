...
// sharedClient is a package-level singleton SpotifyClient.
// All concurrent metadata fetches share one initialized session, preventing
// parallel calls to open.spotify.com/api/token that trigger HTTP 429s.
var (
	sharedClient   *SpotifyClient
	sharedClientMu sync.Mutex
)

// GetSharedSpotifyClient returns the package-level singleton, initializing it
// on first call. If initialization fails the singleton is not cached so the
// next caller will retry. Concurrent callers safely serialize the init via mu.
func GetSharedSpotifyClient() (*SpotifyClient, error) {
	sharedClientMu.Lock()
...
// invalidateSharedSpotifyClient clears the cached singleton so the next call
// to GetSharedSpotifyClient will re-initialize. Call this when a 401/403 is
// returned to force fresh token acquisition.
func invalidateSharedSpotifyClient() {
	sharedClientMu.Lock()
	sharedClient = nil
...
func (c *SpotifyClient) Query(payload map[string]interface{}) (map[string]interface{}, error) {
	...
	if resp.StatusCode != 200 {
		...
		// Invalidate the singleton so the next caller forces a fresh session.
