...
func (t *TidalDownloader) GetTidalURLFromSpotify(spotifyTrackID string) (string, error) {
	...
	fmt.Fprintf(os.Stderr, "Found Tidal URL: %s\n", tidalURL)
	return tidalURL, nil
}
...
func (t *TidalDownloader) GetDownloadURL(trackID int64, quality string) (string, error) {
	fmt.Fprintln(os.Stderr, "Fetching URL...")
	...
	if resp.StatusCode != 200 {
		fmt.Fprintf(os.Stderr, "✗ Tidal API returned status code: %d\n", resp.StatusCode)
	...
	if err := json.Unmarshal(body, &v2Response); err == nil && v2Response.Data.Manifest != "" {
		fmt.Fprintln(os.Stderr, "✓ Tidal manifest found (v2 API)")
	...
	if err := json.Unmarshal(body, &apiResponses); err != nil {
		...
		fmt.Fprintf(os.Stderr, "✗ Failed to decode Tidal API response: %v (response: %s)\n", err, bodyStr)
		return "", fmt.Errorf("failed to decode response: %w (response: %s)", err, bodyStr)
	}

	if len(apiResponses) == 0 {
		fmt.Fprintln(os.Stderr, "✗ Tidal API returned empty response")
		return "", fmt.Errorf("no download URL in response")
	}

	for _, item := range apiResponses {
		if item.OriginalTrackURL != "" {
			fmt.Fprintln(os.Stderr, "✓ Tidal download URL found")
			return item.OriginalTrackURL, nil
		}
	}

	fmt.Fprintln(os.Stderr, "✗ No valid download URL in Tidal API response")
...
func (t *TidalDownloader) DownloadFromManifest(manifestB64, outputPath string) error {
	...
	fmt.Fprint(os.Stderr, "Downloading init segment... ")
	...
	for i, mediaURL := range mediaURLs {
		...
		fmt.Fprintf(os.Stderr, "\rDownloading: %.2f MB (%d/%d segments)", mbDownloaded, i+1, totalSegments)
	}
	...
	fmt.Fprintln(os.Stderr, "Converting to FLAC...")
...
func (t *TidalDownloader) DownloadByURL(tidalURL, outputDir, quality, filenameFormat string, includeTrackNumber bool, position int, spotifyTrackName, spotifyArtistName, spotifyAlbumName, spotifyAlbumArtist, spotifyReleaseDate string, useAlbumTrackNumber bool, spotifyCoverURL string, embedMaxQualityCover bool, spotifyTrackNumber, spotifyDiscNumber, spotifyTotalTracks int, spotifyTotalDiscs int, spotifyCopyright, spotifyPublisher, spotifyURL string) (string, error) {
	...
	fmt.Fprintf(os.Stderr, "Using Tidal URL: %s\n", tidalURL)
	...
	if err := EmbedMetadata(outputFilename, metadata, coverPath); err != nil {
		...
		fmt.Fprintln(os.Stderr, "Metadata saved")
	}

	fmt.Fprintln(os.Stderr, "Done")
	fmt.Fprintln(os.Stderr, "✓ Downloaded successfully from Tidal")
...
func parseManifest(manifestB64 string) (directURL string, initURL string, mediaURLs []string, err error) {
	...
	if err := xml.Unmarshal(manifestBytes, &mpd); err == nil {
		...
		if selectedBandwidth > 0 {
			fmt.Fprintf(os.Stderr, "Selected stream: Codec=%s, Bandwidth=%d bps\n", selectedCodecs, selectedBandwidth)
		}
	}
	...
	if segmentCount > 0 && initURL != "" && mediaTemplate != "" {
		...
		fmt.Fprintf(os.Stderr, "Parsed manifest via XML: %d segments\n", segmentCount)
...
func getDownloadURLParallel(apis []string, trackID int64, quality string) (string, string, error) {
	...
	fmt.Fprintf(os.Stderr, "Requesting download URL from %d APIs in parallel...\n", len(apis))
